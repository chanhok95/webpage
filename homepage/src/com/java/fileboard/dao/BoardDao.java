	...
	public int insert(BoardDto boardDto) {
		...
		try {
			...
			if (boardDto.getFilesize() == 0) {
				String sql = "insert into board(board_number,writer,subject,email,content,password,write_date,read_count,group_number,sequence_number,sequence_level)" // 테이블
																																										// 중간에
																																										// 값이
																																										// 추가될경우
																																										// 명시
				...
				String sql = "insert into board(board_number,writer,subject,email,content,password,write_date,read_count,group_number,sequence_number,sequence_level,file_name,path,file_size)" // 테이블
																																																// 중간에
																																																// 값이
																																																// 추가될경우
																																																// 명시
						+ "values(board_board_number_seq.nextval,?,?,?,?,?,?,?,?,?,?,?,?,?)"; // insert into board
																								// values(board_board_number_seq.nextval,
																								// ?, ?, ?, ?, ?, ?, ?,
																								// ?, ?, ?, ?, ?, ?)
				conn = ConnetionProvider.getConnetion();
...
//		System.out.println(boardDto.toString());
	...
	public BoardDto read(int boardNumber) {
		...
		try {
			...
			pstmt.executeUpdate();
			if (pstmt != null)
				pstmt.close(); // pstmt가 2번 사용되어서 (사용자제뜸) 사용한거 닫고
